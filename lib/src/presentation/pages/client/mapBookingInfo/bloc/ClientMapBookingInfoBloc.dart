    ...
    on<CargoWeightChanged>((event, emit) {
      emit(state.copyWith(
          cargoWeight: BlocFormItem(
              ...
              error:
                  event.cargoWeight.value.isEmpty ? 'Ingresa el peso' : null)));
    });

    on<CargoWeightUnitChanged>((event, emit) {
      emit(state.copyWith(cargoWeightUnit: event.unit));
    });

    on<TruckTypeChanged>((event, emit) {
      emit(state.copyWith(truckTypeRequired: event.type));
    });

    on<CargoTypeChanged>((event, emit) {
      emit(state.copyWith(cargoType: event.type));
    });

    on<HelpersRequiredChanged>((event, emit) {
      emit(state.copyWith(helpersRequired: event.value));
    });

    on<RequiresCraneChanged>((event, emit) {
      emit(state.copyWith(requiresCrane: event.value));
    });

    on<FragileCargoChanged>((event, emit) {
      emit(state.copyWith(fragileCargo: event.value));
    });
